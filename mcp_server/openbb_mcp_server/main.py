...
from .utils.session_config import SessionConfigMiddleware
...
def create_mcp_server(settings: MCPSettings, fastapi_app: FastAPI) -> FastMCPOpenAPI:
    ...
    # Attach middleware to capture per-session configuration from HTTP query params
    try:
        mcp.add_middleware(SessionConfigMiddleware())
    except Exception:
        pass
    ...
    if settings.enable_tool_discovery:
        ...
        @mcp.tool(tags={"admin"})
        def session_config(ctx: Context) -> dict:
            """Return the current session's configuration as seen by the server."""
            return (ctx.get_state("session_config") or {})
